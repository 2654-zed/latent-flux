...
- **Status:** **IN_PROGRESS — partial resolution 2026-05-15**
- **Partial resolution (Confidence: MEDIUM):** Investigation completed in [reports/animoca_deployer_investigation_2026-05-15.md](../reports/animoca_deployer_investigation_2026-05-15.md). Findings: (1) Blockscout OLI tag is specific and curated — `slug: animoca-deployer`, `tagType: name`, `tooltipUrl: https://www.revvmotorsport.com/`. (2) Token-holding corroborates: address holds **REVV (36 transfers) + OFC OneFootball Club (6) + ANIMOCA (2) tokens** — three different Animoca-portfolio assets, hard to explain as coincidence. (3) Adversarial behavior is also confirmed: Mar-26 deploy of 0x752c5a95 on Base, 50 mainnet CREATE2 vanity-deploys on Apr-27, 4,587-victim discharge on May-9. (4) Cross-chain choreography: LayerZero `send()` self-bridge of OFC from Ethereum to Base 50 minutes after the Base trap was deployed — same institutional address used as cross-chain cover. **Prevailing interpretation:** OLI tag is correct; the address is most likely either (a) key-compromised (v3 Attack 11a at brand scale) or (b) controlled by a former Animoca employee. Both are operationally identical from Layer 3's perspective. **Remaining work:** external verification with Animoca Brands; classification of the 50 mainnet CREATE2-deployed contracts.
