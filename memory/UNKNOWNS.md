...
- **Status:** **IN_PROGRESS — methodological resolution 2026-05-16 (Q-003 wired); external attribution pending**
- **Partial resolution (Confidence: MEDIUM-to-HIGH on the operational axis):** Investigation in [reports/animoca_deployer_investigation_2026-05-15.md](../reports/animoca_deployer_investigation_2026-05-15.md). Findings: (1) Blockscout OLI tag is specific and curated — `slug: animoca-deployer`, `tagType: name`, `tooltipUrl: https://www.revvmotorsport.com/`. (2) Token-holding corroborates: address holds **REVV (36) + OFC (6) + ANIMOCA (2) tokens** — three different Animoca-portfolio assets, hard to explain as coincidence. (3) Adversarial behavior is also confirmed: Mar-26 deploy of 0x752c5a95 on Base, 50 mainnet CREATE2 vanity-deploys on Apr-27, 4,587-victim discharge on May-9. (4) Cross-chain choreography: LayerZero `send()` self-bridge of OFC from Ethereum to Base 50 minutes after the Base trap was deployed — same institutional address used as cross-chain cover. **Prevailing interpretation:** OLI tag is correct; the address is most likely either (a) key-compromised (v3 Attack 11a at brand scale) or (b) controlled by a former Animoca employee.
- **2026-05-16 methodological closure:** The Q-003 OLI temporal validity scorer (`surveillance/sai/oli_temporal_validity.py`, landed this session) marks 0x80b12bd0 as **STALE** with score 8.0 (signals: `adversarial_watchlist_high` +5.0, `deployed_confirmed_trap` +3.0). The recommendation is "DO NOT redirect via OLI guardrail. The tag's temporal validity is broken." This is now mechanical — INV-007 can consult Q-003 before applying the guardrail, and the safe-by-accident scenario becomes safe-by-design. Layer 3's defensive posture no longer depends on resolving which of (a)/(b)/(c) is the true cause.
- **Remaining work:** external verification with Animoca Brands (Action #1 from the report) determines WHICH scenario is true. Classification of the 50 mainnet CREATE2-deployed contracts (Action #3) is also pending.
