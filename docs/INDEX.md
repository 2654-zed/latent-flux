...
### `0x752c5a95` Pre-Drain Harvester (Base) — **RETRACTED ENTIRELY 2026-05-21, Correction #24**

> **[CORRECTION #24 — 2026-05-21]** This entire entry is RETRACTED. `0x752c5a95` is **OneFootball Club (OFC)**, a verified Animoca-deployed ERC-20 token (`ERC20FixedSupply` from `@animoca-network/contracts`). 3,904 holders, $7.9M market cap, listed on CoinGecko. The "pre-drain harvester" framing from 2026-04-24 and the "4,587-victim discharge" framing from the 2026-05-21 case file were both produced by three stacked false-positive bugs (bytecode FP on Animoca framework; behavioral FP on pre-launch ERC-20s; `approval_watchlist` crediting failed transferFrom reverts as multi-victim drains). The 2026-05-09 "discharge transactions" were FAILED `transferFrom` calls that moved zero tokens. There is no harvester and there was no discharge. Full root-cause: `reports/correction_log.md` Correction #24. The 2026-05-21 case file `CASE_HARVESTER_DISCHARGE_0x752C5A95_20260509.md` is annotated with a top-of-file retraction notice; content preserved per immutable-corpus-record discipline.

#### Original entry (preserved for historical record — DO NOT CITE):

- ~~**ID:** EXTRACTION_011 (assigned 2026-05-21 retroactively to the discharge event)~~ **EXTRACTION_011 designation withdrawn.**
- ~~**Description:** Confirmed-tier contract that harvested Permit2 approvals from 1,898+ victims (as of 2026-04-24) without firing a sweep. Deployer is `0x80b12bd0` — **Animoca: Deployer** per OLI labels after Correction #20 (was originally framed as pristine-solo with 2019 mainnet vintage). Bytecode flags `has_asymmetric_transfer=1`, `has_unusual_fee_structure=1`.~~ The Animoca-deployer attribution is real (Blockscout mainnet returns `animoca-deployer`, `contract-deployer`, `animoca` tags). The "harvester" framing is the false positive. The bytecode flags fire on the Animoca framework's standard `ContractOwnership` + `TokenRecovery` patterns.
- ~~**2026-05-09 discharge event:** Two independent drain_caller EOAs swept the harvester within 30 minutes. `0x1d81aff2a24c` drained 3,228 victims in a single tx at 11:28:23Z; `0x0e2224685fe7` drained 1,359 victims across 11:50:05Z–11:58:01Z. **Total: 4,587 unique victims drained in 30 minutes.** Validated the 2026-04-24 Tier-C prediction within 15 days.~~ Both "discharge" transactions reverted (status=error, gas ~25K). Zero tokens moved. The 4,587 victim count is phantom rows generated by the pipeline bug.
- ~~**Status:** DISCHARGED~~ **CORRECTED STATUS: RETRACTED. The contract is a legitimate ERC-20 token.**
...
### ~~EXTRACTION_011 — `0x752c5a95` Harvester discharge wallets (2026-05-09)~~ **RETRACTED 2026-05-21 — Correction #24**
- ~~`0x1d81aff2a24c822d715ec09a0f81801face6e6fd`~~ — Per Blockscout: an EIP-7702 smart wallet (Coinbase smart-wallet implementation), ENS name `jeephoenix.eth`. Personal user wallet. Its 2026-05-09T11:28:23Z transaction on `0x752c5a95` was a FAILED `transferFrom` (reverted, status=error, zero tokens moved). The "3,228 drained victims" attribution is phantom rows from a pipeline bug. **Do not add to watchlist. Not adversarial.**
- ~~`0x0e2224685fe775b471b457c643913e4bbd66c8d2`~~ — Per Blockscout: plain EOA, first tx 2025-03-02. Its two transactions on `0x752c5a95` on 2026-05-09 were FAILED `transferFrom` calls (reverted, status=error, zero tokens moved). The "1,359 drained victims" attribution is phantom rows from the same pipeline bug. **Do not add to watchlist. Not adversarial.**
...
### KelpDAO LayerZero exploit (EXTRACTION_008, 2026-04-18) — expanded 2026-05-22 from LayerZero Labs report
- `0x85d456B2DfF1fd8245387C0BfB64Dfb700e98Ef3` — Kelp rsETH OFTAdapter (Ethereum). Adapter that minted 116,500 rsETH on the forged attestation. — `reports/extraction_event_008_kelp.md`, `reports/kelp_retrospective_replay.md`.
- `0x589dEDbD617e0CBcB916A9223F4d1300c294236b` — LayerZero Labs DVN (Ethereum). The DVN whose signing service was tricked into attesting via RPC poisoning. Keys never compromised; the RPC layer the DVN read from was poisoned via in-memory `op-geth` patching (CARVED1/CARVED2 ELF PIE malware). — `reports/extraction_event_008_kelp.md`.
- `0x282b3386571f7f794450d5789911a9804fa346b4` — LayerZero Labs DVN (Unichain). Counterpart on source chain. — `reports/extraction_event_008_kelp.md`.
- **`0x1f7A03b70C5448DFd0a2C5a7865169253c2C769b`** — **OApp delegate (single EOA)**. The key that modified the channel security configuration from 2-of-2 to 1-of-1 before the exploit. Single EOA controlled the security policy of a $292M bridge. Added to watchlist HIGH 2026-05-22 as `kelp_oapp_delegate_config_authority`. — `reports/extraction_event_008_kelp.md`. **Lexicon connection**: canonical case for [Configuration-Level Vulnerability](lexicon.md#configuration-level-vulnerability) and [Operator-Layer Cross-Chain Compromise](lexicon.md#operator-layer-cross-chain-compromise) lexicon entries.
- `0x1a44076050125825900e736c501f859c50fE728c` — LayerZero EndpointV2 (Ethereum, immutable). The endpoint that accepted the forged attestation per the application-configured 1-of-1 policy. Not malicious — protocol-layer code executed correctly per the configuration. — `reports/extraction_event_008_kelp.md`.
- `0x8B1b6c9A6DB1304000412dd21Ae6A70a82d60D3b` — attack recipient (fresh address). Funded via Tornado Cash 6.5h pre-exploit; received 116,500 rsETH; fanned out to 7 downstream wallets within 98 seconds. — `reports/kelp_retrospective_replay.md` Phase 4.
- **Attribution (2026-05-22, Tier A):** all addresses above attributed to **UNC4899 / TraderTraitor** (DPRK, aligned with Reconnaissance General Bureau) by Mandiant + CrowdStrike (high-confidence DPRK, medium-confidence UNC4899) and independently by tanuki42 + tayvano via funding de-mixing. Same threat actor as the Bybit Safe{Wallet} heist (Feb 2025, $1.5B). See [UNC4899 / TraderTraitor](lexicon.md#unc4899--tradertraitor) lexicon entry + `reports/threat_actor_iocs_layer3.md` for off-chain IOCs (malicious GitHub, malware hashes, C2 domains).
