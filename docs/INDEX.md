...
### Active pre-stage warehouse — `0xc43f317e` (case file 2026-05-10)
- **Funder:** `0xc43f317ed4d81cbbfe2c9c98b4cc6f303519f078` — Base, **2,535 downstream deployer wallets** (up from 1,562 at April-25 ISO snapshot, **+62% in two weeks**). OLI-clean per 2026-05-09 audit. Mainnet first-tx 2025-11-09 (~6 months old).
- **Operating pattern:** 49-day operational window (2026-03-23 → 2026-05-10, ongoing). 815 downstream deployers active in the last 14 days. Sustained ~95 new deployers per active burst day. Latest deployment 2026-05-10T15:20:59 (5 minutes ago at probe time).
- **Bytecode signature:** **100% concentration on a single hash `49155b60033de73770...`** across 1,206 hashed contracts. Zero adversarial bytecode flags (`asym=0 cond_revert=0 unusual_fee=0`). All 2,366 known downstream contracts remain in `unknown` confidence tier.
- **Realized extraction: zero.** 0 `trap_events`, 0 `approval_watchlist` entries, 0 `extraction_events` against any downstream contract.
- **Status:** ACTIVE — strongest stored-potential signal in the corpus not already retracted or organization-attributed. Stored Potential framework reads: HIGH on structural primitives (fanout × template concentration × operational continuity), ZERO on activation primitives (no approvals, no realized value). The closest analog is the Dragon (`0x2e20b26172a8`, 2,077 pre-staged ERC-20s) but Dragon's contracts have pre-approvals; c43f317e's do not — earlier life-cycle stage if a stockpile.
- **Primary case file:** `surveillance/data/cases/CASE_PRESTAGE_WAREHOUSE_0xc43f317e.md` (2026-05-10).
- **Watchlist:** Not yet tier-assigned; case file recommends MEDIUM pending bytecode decompilation of the dominant template.
- **Top-12 ISO context:** This is one of the 4 OLI-cleared unattributed Top-12 entries from Correction #20's deeper audit (4 of 12 are CEX/bridge; 4 are pre-attributed adversarial; 4 remain as residual ISO topology — c43f317e is the largest of the residual 4 and the only one still actively growing).
- **Open question:** decompile `49155b60033de73770...` — without knowing what the bytecode computes, the typology cannot be sharpened beyond "infrastructure-scale single-template depositor."
