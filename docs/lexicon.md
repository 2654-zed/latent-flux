...
- **Layer 3 corpus instance — bb50 industrial-scale stockpile (`0xbb50ce87be3443ed137df1dfdbf2fb0ca8c0a9e0`, Optimism):** 38,016 deployed contracts as of 2026-05-02, 0 lifetime drains, 0 lifetime approvals against the fleet (per `surveillance/data/cases/...` cross-link probe). The operator has burned non-trivial Optimism gas on contract creation across ~21 days with zero realized extraction. **Current CER ≈ 0.** This is thermodynamically a pre-funding state: either the stockpile activates and CER goes positive, or capital depletes and the operation collapses. The framework predicts which way it resolves before the on-chain extraction signal appears.
- **Layer 3 corpus contrast — drainer-spawn hubs `0xf7883e3fef23` and `0x3304e22ddaa2`:** unlike bb50's stockpile (CER ≈ 0), these hubs exhibit observable positive CER. `0xf7883e3fef23` spawned 6 drainer wallets across 24 days for 859 victims drained; `0x3304e22ddaa2`'s April 1 iteration alone drained 399 victims in 4 days. The compute cost (gas to deploy 6-7 drainer contracts + drain transactions) is bounded; the extraction is bounded but materially larger. CER is positive and the operation is sustainable on its own thermodynamics. The difference between bb50 and these hubs is not strategy diversity — it's thermodynamic state: stockpiles run negative-CER on capital advance betting on future activation; hubs run positive-CER on per-iteration extraction.
