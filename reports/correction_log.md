...
10. **Add `approev` function-name signature** (or a more robust hidden-function detector) to Layer 3's bytecode classifier. The 8ca70232 sample contract was correctly flagged `confirmed` by the existing classifier, but 319 of 320 contracts in the fleet (carrying the same predatory primitives) sit at `suspected` or `unknown`. A function-name-based heuristic is brittle but catches the operator-class until they rotate the naming.

11. **Cross-reference the 5 hardcoded blacklist addresses** baked into `0xaeac0e69`'s bytecode (`0x1f2F10D1C...`, `0xae2Fc483...`, `0xC38e00aC...`, `0x01D37a36...`, `0x93C7878c...`) against other Layer 3 entries. The pattern of WHO gets blacklisted across the 320-token fleet may reveal operator targeting strategy and surface a victim list of ~1,600 addresses.
