# Potential Attacks v3 — Combinatorial Threat Models After the May 2026 Wave

**Revision date:** 2026-05-15
**Predecessor:** `POTENTIAL_ATTACKS_V2_ARCHIVE.md` (dated 2026-04-20; preserved verbatim as an evidence record).
**Predecessor's predecessor:** `POTENTIAL_ATTACKS_V1_ARCHIVE.md` (2026-04-07).

Attack scenarios constructed by combining patterns Layer 3 has observed in the corpus. Each scenario is hypothetical at the **combination** level; every individual component has corpus evidence. The April 2026 exploit wave — Drift (Apr 1), Silo (Apr 3), Aethir (Apr 9), Zerion (Apr 10), Hyperbridge (Apr 13), Dango (Apr 13), Rhea (Apr 16), Kelp (Apr 18) — validated several v1 attacks, introduced new categories that v1 did not anticipate, and sharpened the vocabulary used for all of them. The late-April / May 2026 follow-on wave — Volo (Apr 21), Wasabi (Apr 30), Renegade (May 10–11), THORChain (May 15), plus smaller late-April incidents (Juicebox V3, Thetanuts) — extends the same combinatorial patterns and validates them at scale; **no new attack category required so far**, with Attack 15 (Native-Swap Bridge Drain) flagged as a candidate pending THORChain mechanism clarification.

**Format (unchanged from v2):**
...
**What's new in v3:**
1. A "Validated in late April – May 2026" section adding Wasabi, Renegade, THORChain, Volo, Juicebox V3, Thetanuts cross-referenced against existing attack categories.
2. Attack 11 split into **11a (key-compromise mode)** and **11b (acquired-admin-via-code-defect mode)** to reflect the Renegade mechanism, which produces the same downstream amplification as Aethir/Wasabi but with a categorically different upstream defense locus.
3. Updated detection hooks for Attack 11 to add **initializer-status enumeration** as a categorically new pre-deploy on-chain check (catches the 11b class proactively).
4. Status count refresh: from v2's 6 observed / 5 components / 3 unconfirmed to v3's 8 observed / 5 components / 3 unconfirmed.
5. Attack 15 candidacy noted (Native-Swap Bridge Drain; pending THORChain mechanism forensics).

---

## Validated in April 2026 [preserved from v2]
...
## Validated in late April – May 2026 [NEW IN v3]

The late-April / mid-May wave continued the April pattern rather than introducing new architectures. Every event below maps cleanly onto an Attack 9–13 category. The most notable framework-level change is **Attack 11's split into 11a and 11b**, forced by the Renegade mechanism.

| Event | Date | Loss | Chain(s) | Maps to | Defining feature |
|---|---|---|---|---|---|
| Volo Protocol | 2026-04-21 | ~$3.5M | (one-chain — TBD) | Attack 11 / 13 | Admin-compromise pattern; full mechanism enumeration pending |
| **Wasabi Protocol** | **2026-04-30** | **~$5M+** | **Ethereum + Base** | **Attack 11a** | **Deployer EOA key compromise → ADMIN_ROLE grant → UUPS proxy upgrade on perp vaults/LongPool → drain user funds + treasury. Blockaid called it "admin-key compromise exploit" with the deployer EOA as the vector. First multi-chain instance of Attack 11.** |
| Juicebox V3 | late April 2026 | (pending) | (TBD) | Continuation; access-control / admin-pattern variant |
| Thetanuts Finance | 2026-04-20 to 30 | (pending) | (TBD) | Continuation; cross-chain / admin-pattern variant |
| **Renegade Finance** | **2026-05-10/11** | **$209K (returned via 90/10 whitehat)** | **Arbitrum** | **Attack 11b** | **Unprotected initializer on legacy Dark Pool proxy contract. Attacker called `initialize()` to set themselves as admin, then used `delegatecall` to inject malicious logic into the proxy's storage, draining 27 ERC-20 tokens. Protocol offered on-chain 90/10 whitehat bounty; most funds returned. Blockaid caught the exposed initializer immediately. Code-defect-acquired-admin path is the categorically new finding.** |
| **THORChain** | **2026-05-15** | **$7.4M–$10M+** | **BTC + ETH + BNB + Base** | **Attacks 9 + 10 + 11** | **Cross-chain native-swap protocol drain. Protocol immediately suspended trading and activated the Mimir governance module to freeze everything. Exact mechanism (DVN-equivalent compromise vs proof bypass vs validator key compromise) TBD pending forensics. Validates the cross-chain pooled-custody surface and the kill-switch governance pattern as a mitigation precedent. Candidate for new Attack 15 (Native-Swap Bridge Drain) if architecturally distinct from LayerZero OApp (Kelp) and MMR-based bridges (Hyperbridge).** |

**Mimir freeze as defensive precedent.** THORChain's Mimir governance module allows the active validator set to set network-wide flags (e.g., `HALTTRADING=1`, `MAXSYNTHPERPOOLDEPTH=0`) without a code upgrade — a parameter-level kill switch. This is the first instance in the v2/v3 corpus of a protocol stopping bleeding mid-incident via on-chain governance rather than off-chain coordination. Protocols with no equivalent surface (Wasabi, Renegade, Aethir) had to absorb the full loss. Lexicon candidate: **Kill-Switch Governance** as a defensive primitive worth measuring per protocol.

**Pattern of the May wave: confirmation, not novelty.** v3's headline finding is that **four new exploits map onto existing v2 categories without forcing a new category**. v2's combinatorial framework, built two weeks before this wave, predicted the surface that produced these events. The framework is load-bearing: it would have flagged Wasabi (EOA-admin perp adapter), Renegade (unprotected-initializer proxy), and the cross-chain surface generally, **before** the events occurred. The v3 update reflects observations, not retro-fits.
...
**Why this didn't happen in April or May.** None of the documented April or May events used coordinated time-flip. Drift and Rhea used configuration vulnerabilities, but each event exploited a single configuration state, not a cross-fleet synchronized flip. The signature Layer 3 is watching for — many contracts sharing T — would be distinguishable from all observed events.
...
**Open work:** building the T-value calendar remains the highest-leverage addition. Nothing in the April or May wave gave us a forcing example, so this stays at the top of the speculative work list.
...
- **Wasabi (2026-04-30) as the first multi-chain extension of the single-contract variant** — Wasabi compromised the deployer EOA, granted ADMIN_ROLE, and upgraded perp vault implementations across Ethereum and Base simultaneously. Not "fleet-coordinated" in the v1 sense (no large pool of dormant proxies), but a deliberate cross-chain version of Attack 11 — the operational layer was compromised once and exploited on two chains in close succession.
...
- **Single-contract high-stored-potential adapters where admin is an EOA without multisig or timelock** — enumerate these and treat them as pre-validated Aethir-class risk
- **Cross-chain admin-identity correlation:** when the same EOA holds admin on adapters on multiple chains, treat the operational-layer compromise risk as multiplicative across the chains (Wasabi's lesson)

**Status: single-contract variant observed (EXTRACTION_006, Wasabi), fleet-coordinated variant components observed, assembly hypothetical. Epistemic Tier A** for Aethir + Wasabi; **Tier B** for fleet-coordinated.
...
## Part III — New categories the April wave established (v2-original) — Updated with v3 instances
...
- **THORChain (2026-05-15, $7.4M–$10M+)** [v3]: cross-chain native-swap protocol, funds drained across BTC / ETH / BNB / Base. THORChain's architecture is materially different from LayerZero OApp — uses its own L1 + per-chain vaults with validator-attested releases — but the **pooled-custody amplification surface** is identical to Attack 9. If the THORChain mechanism turns out to be validator-signature forgery (single-validator-equivalent attestation), it fits Attack 9. If it's a code-level validator-set issue, Attack 10 applies. If it's an operational-layer validator-key compromise, Attack 11 applies. Mechanism TBD; flagging as candidate for **Attack 15 (Native-Swap Bridge Drain)** if architecturally distinct enough to warrant a category split.
...
- **[v3] Validator-set size monitoring on non-LayerZero cross-chain protocols.** THORChain's potential exposure suggests this hook should generalize: enumerate cross-chain protocols' attestation thresholds (DVN, validator-set, council, M-of-N keys) and treat thresholds with single-point-of-failure surfaces as a pre-validated category regardless of protocol stack.

**Status: observed (EXTRACTION_008; THORChain pending mechanism). Epistemic Tier A** for the configuration-level observation; **Tier B** for the generalization across all cross-chain attestation systems.
...
- **THORChain (2026-05-15)** [v3]: if mechanism turns out to be proof / attestation forgery rather than key compromise, this maps here. See Attack 9 note for mechanism TBD status.
...
**Status: observed (EXTRACTION_007; THORChain pending). Epistemic Tier A** for the incident; **Tier B** for the generalization across bridge architectures.

---

### Attack 11 — Pooled Custody Adapter Compromise (Aethir category)

**v3 split:** Attack 11 has two sub-categories that produce the same downstream amplification (pooled custody → admin authority → drain) but require different upstream defenses. The split was forced by Renegade (2026-05-10/11), where the attacker did not compromise an existing admin key — they exploited a code-level vulnerability to **become** the admin.

#### Attack 11a — Key-compromise mode (Aethir, Wasabi, Volo)

**Storing potential.** OFT adapter, lock-and-release adapter, or perp vault deployed with an EOA admin (no multisig, no timelock). Adapter accumulates user deposits in pooled custody. Admin key sits on a single device / in a single engineer's wallet.

**Seeding.** The TVL accumulates over weeks or months. The admin privilege includes the ability to mint, pause, withdraw, or upgrade. The admin identity is typically a matter of public record (deployer address is on Etherscan) and the private key is in the custodian's operational setup.

**Discharge.** Private key compromised via (a) phishing of the keyholder, (b) malware on the keyholder's device, (c) supply-chain attack on wallet software, (d) social engineering a key export, or (e) direct insider action. Attacker uses the authorized admin function to mint-or-drain the pool. The on-chain signature is cryptographically valid; the code enforces exactly what the signature authorizes.
...
- **Wasabi Protocol (2026-04-30, ~$5M+) [v3].** Ethereum + Base, perp vaults / LongPool. Attacker compromised deployer EOA, granted themselves ADMIN_ROLE, then used a UUPS proxy upgrade to drain user funds and treasury. Blockaid flagged it as an "admin-key compromise exploit" with the deployer EOA as the vector. First multi-chain instance of Attack 11 — same operational compromise, two-chain coordinated drain.
- **Volo Protocol (2026-04-21, ~$3.5M) [v3].** Continuation of the admin-compromise pattern; full mechanism enumeration pending.
- **EXTRACTION_005 (Drift, 2026-04-01, ~$285M)** is the governance-scale cousin; see Attack 13.
- **Zerion DPRK attack (2026-04-10, ~$100K)**: small-scale social-engineering variant.
- **Bybit ($1.5B, 2025)**: the canonical large-scale operational-layer attack precedent.

#### Attack 11b — Acquired-admin-via-code-defect mode (Renegade)

**Storing potential.** Adapter or proxy contract deployed with the admin set to `address(0)` until an `initialize()` call. The initializer is intended to be called once during deployment to establish the admin. If the deployment script fails to call `initialize()` immediately AND access-control on `initialize()` is insufficient (no `onlyOwner` gate, no `_disableInitializers()` call in the constructor of the implementation), the function remains callable by anyone.

**Seeding.** The proxy holds value or governs value. Users interact with it under the assumption that it has a legitimate admin. Time passes; the unprotected initializer sits as a latent foothold.

**Discharge.** Attacker calls `initialize()` to set themselves as admin / owner. Once admin, they have the full authorized-upgrade path: they can replace the implementation with a malicious contract via `delegatecall`, draining the proxy's storage. The on-chain signatures are all cryptographically valid; every call executes exactly what the code allows. The bug is that the code allowed the takeover.

**Lexicon framing.** 11b sits between [Compositional Harm](docs/lexicon.md#compositional-harm) (the code as deployed is honest in isolation but unsafe in context) and a traditional smart-contract bug. The upstream defense locus is **pre-deployment audit** — initializer protection is a known, named, audit-catchable issue (`OpenZeppelin/openzeppelin-contracts#3030` and many advisories) — but the on-chain detection surface ALSO exists, which is the categorically new finding: every proxy in a corpus can be enumerated and checked for "is initialize callable by any address right now?". This is a Tier A on-chain check.

**Confirmed instance:**
- **Renegade Finance (2026-05-10/11, $209K on Arbitrum) [v3].** Unprotected initializer on legacy Dark Pool proxy contract. Attacker called `initialize()` to set themselves as admin, then used `delegatecall` to inject malicious logic into the proxy's storage, draining 27 different ERC-20 tokens. Protocol offered an on-chain 90/10 whitehat bounty and most funds were returned. Blockaid caught the exposed initializer immediately — meaning the on-chain detection was available pre-exploit, but no continuous-monitoring product was running it on this specific contract before the attacker did.

**Why the split matters for defense:**
- 11a's defense locus is **operational security** (multisig, hardware wallets, timelock, separation of duties) — not auditable on-chain.
- 11b's defense locus is **pre-deployment code audit** + **on-chain initializer-state monitoring** — fully auditable and on-chain detectable.
- Both share the **same downstream amplification** (pooled custody → admin authority → drain), so the on-chain monitor for "sudden admin-invoked large transfer that drains >X% of custody" catches both variants at the discharge moment.

#### Attack 11 (combined) — Why it's hard to see

- No code is defective from the protocol's perspective at the moment of exploit (11a) OR the defect is a known-named class but isn't caught by the continuous monitor (11b).
...
- For 11a, defense lives in operational security, which is not auditable from on-chain data.
- For 11b, defense lives in initializer-state monitoring, which exists as a category but is not standard scanner equipment.

#### Attack 11 (combined) — Detection hooks

- **`infrastructure_registry`-based enumeration of all high-TVL single-admin adapters.** Any adapter with an EOA admin, no multisig, and a pooled-custody mechanism is pre-validated as 11a-class risk.
...
- **[v3] Initializer-status enumeration** (catches 11b proactively): for every proxy contract in the corpus, check whether `initialize()` (or equivalent) has been called with a legitimate admin AND whether subsequent `initialize()` calls revert (e.g., the `_disableInitializers()` or `initialized` storage slot check). Any proxy where `initialize()` remains callable by an arbitrary address is a pre-validated Renegade-class risk.
- **[v3] Cross-chain admin-identity correlation** (catches multi-chain 11a like Wasabi): when the same EOA is admin on adapters across multiple chains, treat the operational-layer compromise risk as multiplicative — a single phishing event compromises every chain at once.

**Status: 11a observed (EXTRACTION_006, EXTRACTION_005, Zerion, Wasabi, Volo). 11b observed (Renegade). Epistemic Tier A** for both.
...
- **No v3 additions** — the May wave did not produce new instances of this category. The Attack 12 surface remains live and the v2 detection hooks are unchanged.
...
- **No direct v3 additions** — May-wave events were operational/code-level (Attacks 11a/11b), not governance-scale. Wasabi was a single-EOA compromise rather than a multi-signer council action. The category remains live and the surface unchanged.
...
- **Zero confirmed advisor-parasites in current corpus.** Corpus age ≤ 60 days is still too short for the months-long extraction signature. `approval_events` doesn't index outbound Transfer flows (only approvals to flagged spenders).
...
## Part IV — v3 candidate category (pending forensics)

### Attack 15 (CANDIDATE) — Native-Swap Pooled-Custody Bridge Drain (THORChain category)

**Status: CANDIDATE, not promoted.** Whether this becomes a standalone attack depends on the post-mortem mechanism for THORChain's 2026-05-15 incident. Three resolution paths:

1. **Mechanism = validator-set attestation forgery** → maps to Attack 9 (DVN equivalent on a non-LayerZero stack). Promote a sub-section of Attack 9 rather than a new attack.
2. **Mechanism = proof / cryptographic-attestation code defect** → maps to Attack 10.
3. **Mechanism = validator-key operational compromise (one or more validator keys leaked / coerced)** → maps to Attack 11a at the validator-set level — possibly with a category extension noting that validator-set compromise has different defense surface than single-keyholder compromise.

If, however, THORChain forensics reveal a fourth pattern — e.g., a native-swap-specific economic exploit where the cross-chain "settle the trade" path has an internal asymmetry not present in LayerZero/MMR/lock-and-release bridge architectures — Attack 15 becomes its own category. Specifically: native-swap protocols match deposits on chain X to releases on chain Y via the protocol's own pooled liquidity; the surface is "any imbalance between deposit verification and release authorization."

**Pending resolution. Will update v3.1 once THORChain post-mortem clarifies the mechanism.**

**Tentative lexicon framing.** [Pooled Custody Amplification](docs/lexicon.md#pooled-custody-amplification) (the pools on every chain are the amplification mechanism), [Verification-Path Trust Failure](docs/lexicon.md#verification-path-trust-failure) (the verification path is "the THORChain validator set agreed"), and possibly a new entry [Native-Swap Asymmetry](docs/lexicon.md#) if mechanism #4 above is confirmed.

**Defensive precedent observed: Kill-Switch Governance.** THORChain's Mimir module allowed the validator set to set `HALTTRADING=1` mid-incident, freezing the protocol parameter-by-parameter without a code upgrade. This is the first v3-era instance of a protocol stopping mid-bleed via on-chain governance. Worth a separate lexicon entry — would generalize to "what fraction of cross-chain protocols have a parameter-level kill switch reachable in <1 hour."

---

## Cross-cutting notes

**Why a v3 now.** The April 2026 wave (eight major exploits across 18 days) was already a load-bearing forcing function for v2. The late-April / May follow-on wave (four more events, ~$15-22M+ total) tested whether v2's framework continued to be load-bearing or had slid into staleness. **It held.** No new attack category was forced; one sub-split (Attack 11a vs 11b) was required to keep the defense-locus distinction clean; one candidate category (Attack 15) is parked pending forensics. The v3 update therefore primarily extends the "Validated" tables and threads in three new detection hooks (initializer-status enumeration, cross-chain admin-identity correlation, validator-set size monitoring on non-LayerZero attestation systems).

**Pattern of the May wave: confirmation, not novelty.** v3's headline finding is that four new exploits map onto existing v2 categories without forcing a new category. v2's combinatorial framework, built two weeks before the May wave, predicted the surface that produced these events. Pre-deployment, the framework would have flagged Wasabi (EOA-admin perp adapter), Renegade (unprotected-initializer proxy), and the cross-chain surface generally. This is the load-bearing claim: **the framework's predictions retroactively match the next wave of incidents** — a stronger validation than v2 could make on its own April-wave-of-record.
...
**v3 status count (14 attacks + 1 candidate):**
- **Observed: 8 attacks** (Attacks 2-single-contract, 5, 9, 10, 11a, 11b, 12, 13)
- Components observed, chain hypothetical: 5 attacks (Attacks 1, 2-fleet, 4, 7, 14)
- Architecturally valid, unconfirmed: 3 attacks (Attacks 3, 6, 8)
- **Candidate: Attack 15** (Native-Swap Bridge Drain — promotion pending THORChain mechanism)

v3 increased observed count by 2 (added 11b as its own observed category; Attack 11a observation extended to Wasabi + Volo). v2's 6 observed → v3's 8 observed without any retraction.

**Epistemic tier discipline.** Every attack continues to carry an explicit [Epistemic Tier](docs/lexicon.md#epistemic-tier-classification) tag in the Status line. Tier A claims are load-bearing in pitches and external materials. Tier B claims are for methodology discussion. Tier C claims are never cited in commercial materials without explicit framing as prediction.

**Relation to the correction log.** Every retraction or revision of a v3 attack's Status (observed → unconfirmed, or the reverse) is a correction-log-triggering event. The 11a/11b split is NOT a retraction — it's a refinement; the v2 Attack 11 entry remains fully observed, and v3 splits it into two valid sub-modes. The split warrants a NEW lexicon entry rather than a correction log entry.

---

## Appendix — v2 → v3 crosswalk

| v2 | v3 | Change |
|---|---|---|
| 1 Permission Harvesting + Routing Parasite | 1 (Part I) | Preserved |
| 2 Dormant Fleet + Proxy Upgrade Swap | 2 (Part II) | Preserved; observed-component list extended with Wasabi as multi-chain single-contract instance |
| 3 TaaS Template with Hidden Skim | 3 (omitted from Part I; preserved in archive) | Unchanged |
| 4 Time-Lock Synchronized Fire | 4 (Part I) | Preserved |
| 5 Cross-Chain Rotation Evasion | 5 (Part II) | Preserved |
...
| 9 Cross-Chain DVN Verification Failure | 9 (Part III) | Updated: THORChain added as pending instance; new detection hook for validator-set size monitoring |
| 10 Cross-Chain Proof Verification Bypass | 10 (Part III) | Updated: THORChain added as pending instance |
| 11 Pooled Custody Adapter Compromise | **11a + 11b (Part III)** | **SPLIT.** 11a = key-compromise mode (Aethir, Wasabi, Volo). 11b = acquired-admin-via-code-defect mode (Renegade). New detection hook: initializer-status enumeration. New detection hook: cross-chain admin-identity correlation |
| 12 Oracle Manipulation via Fake-Token Collateral | 12 (Part III) | Preserved (no May-wave instances) |
| 13 Operational Layer Compromise at Governance Scale | 13 (Part III) | Preserved (no May-wave instances) |
| 14 Advisor-Parasite Pattern | 14 (Part III) | Preserved (no May-wave instances; corpus age now ~60 days, still under the 90-day threshold) |
| — | **15 (CANDIDATE)** Native-Swap Pooled-Custody Bridge Drain | **NEW — pending THORChain forensics** |

---

## Appendix — v1 → v2 crosswalk [preserved from v2]
